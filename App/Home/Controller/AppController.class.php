...
class AppController extends Controller
{
	...
	public function indexcont(){
		$mianfei=M('mv_movies')->field('ID,MV_NAME,MV_PHOTO_URL')->where('MV_TYPE=1  and IS_DEL=0 and MV_STATUS=1 and MV_PHYLETIC=3')->order('GMT_MODIFED desc')->limit(2)->select();
		//echo M('mv_movies')->getlastsql();die();
		if(empty($mianfei)){
			$mianfei=0;
		}
		$da['mianfei']=$mianfei;
		$new=M('mv_movies')->field('ID,MV_NAME,MV_PHOTO_URL,MV_TIME,MV_TYPE')->where(' IS_DEL=0  and MV_STATUS=1 and MV_PHYLETIC=3')->order('ID desc')->limit(4)->select();
		if(empty($new)){
			$new=0;
		}
		$da['new']=$new;
		$jingxuan=M('mv_movies')->field('ID,MV_NAME,MV_PHOTO_URL,MV_TYPE')->where(' IS_DEL=0  and MV_STATUS=1 and MV_PHYLETIC=2')->order('MV_PLAY_COUNT desc')->limit(4)->select();
		if(empty($jingxuan)){
			$jingxuan=0;
		}
	...
	public function hotmovie(){
		...
		$ad=M('mv_movies')->field('ID,MV_NAME,MV_PHOTO_URL,MV_TIME,MV_TYPE')->where(' IS_DEL=0 and MV_PHYLETIC=3 and MV_STATUS=1')->order('ID desc')->limit($pageindex,$pagesize)->select();
		if(empty($ad)){
			$ad=0;
		}
		$this->returnjson('0','成功！',$ad);
	...
	public function jingmovie(){
		...
		$pageindex=$_GET['pageindex'];
		$pageindex=$pageindex*$pagesize;
		$ad=M('mv_movies')->field('ID,MV_NAME,MV_PHOTO_URL,MV_TYPE')->where(' IS_DEL=0 and MV_PHYLETIC=2 and MV_STATUS=1')->order('ID desc')->limit($pageindex,$pagesize)->select();
		if(empty($ad)){
			$ad=0;
		}else{
			$uid=$_GET['uid'];
	...
	public function moviedec(){
		...
		if(!empty($uid)&&$uid!=0){
			...
			if($movie['MV_TYPE']==2){
				...
				if(empty($vip)){
					...
					if(empty($son)){
						...
						$classid=$movie['MV_PHYLETIC'];
						$islook=M('mv_look_history')->where("LH_PHYLETIC=1 and LH_LOOKID=$id and LH_USERID=$uid and TYPE=1 and GMT_MODIFED>'$totime' and LH_CLASS=$classid")->find();
						if(empty($islook)){
							$looknum=M('mv_look_history')->where("LH_TYPE=2 and LH_PHYLETIC=1 and LH_USERID=$uid and TYPE=1 and GMT_MODIFED>'$totime' and LH_CLASS=$classid")->count();
							//分类
							$class=M('mv_class')->find($classid);
							
							if($looknum>=$class['CL_NUM']){
								 $movie['shikan']=1;
								 if($cn==1){
									unset($movie['MV_HURL']);
									unset($movie['MV_WURL']);
								...
								if($class['CL_NUM']==0){
									...
									$this->returnjson('3','您今天已观看'.$class['CL_NUM'].'部'.$class['CL_NAME'].'，请充值或推荐新会员！',$data);
								}
			...
			if(!empty($look)){
				...
				$look['LH_PHYLETIC']=1;
				$look['LH_CLASS']=$movie['MV_PHYLETIC'];
				$look['LH_LOOKID']=$id;
				$look['TYPE']=1;
	...
	function signurl($urlw){
		...
		$host=$urlw['host'];
		$buket=explode('.',$host);
		if(isset($buket['2'])&&$buket['2']=='aliyuncs'){
			 $bucketname=$buket['0'];
		...
		//http://ceshi2323.oss-cn-beijing.aliyuncs.com/029.mp4
	...
	public function cartoondec(){
		...
		$cartoon['CT_PHOTO_URL']=json_decode($cartoon['CT_PHOTO_URL']);
		//评论总数
	...
	public function  zhangdesc(){
		...
		$zhang['CTD_PHOTO_LIST']=json_decode($zhang['CTD_PHOTO_LIST']);
		if(!empty($uid)&&$uid!=0){
			if($zhang['CTD_TYPE']==2){
				$this->checkvip($uid);
			}
	...
	function  pay(){
		...
		//支付虎皮椒账号
		$hupijiao=M('mage_hupijiao')->where("IS_DEL=0 and `MAX_MONEY`>(`NOW_MONEY`+$money)")->order('rand()')->find();
		if(empty($hupijiao)){
		...
		if(!empty($oid)){
			...
			$data['notifyUrl'] = 'http://shiping.l.100help.net/index.php/app/notify.html';
	...
	public function classcont(){
		...
		if($type==1){
	    	$ad=M('mv_movies')->field('ID,MV_NAME as name,MV_PHOTO_URL as pic,MV_TYPE as type,MV_CONTENT as content')->where(" IS_DEL=0 and MV_STATUS=1 and MV_PHYLETIC=$cid")->order('ID desc')->limit($pageindex,$pagesize)->select();
			if(empty($ad)){
				$ad=0;
			}
		}else{
