                 ...
                 <div class="layui-form-item">
                      ...
                      <div class="layui-input-block">  
                        ...
                        <select name="modules" id="classid" lay-verify="required" lay-search="">
                        	 ...
                        	 <?php if(is_array($data["class"])): $i = 0; $__LIST__ = $data["class"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$da): $mod = ($i % 2 );++$i;?><option value="<?php echo ($da["ID"]); ?>"  <?php if(isset($data['MV_CATEGORY'])&&($data['MV_CATEGORY']==$da['ID'])): ?>selected<?php endif; ?>><?php echo ($da["CL_NAME"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                      </div>
                    </div>
         ...
         //自定义验证规则 
		  ...
		  //alert(111);
			...
			$.ajax({
				 ...
				 data: {name:$('#name').val(),mvtime:$('#mvtime').val(),classid:$('#classid').val(),d_href:$('#d_href').val(),sd_href:$('#sd_href').val(),lei:$('#lei').val(),type:type,pic:$('#spic').val(),'w_href':$('#w_href').val(),'sw_href':$('#sw_href').val(),click:$('#click').val(),desc:$('#desc').val(),id:$('#upid').val(),pg:$('#pg').val()},
				 dataType: "json",
				 async:false,
